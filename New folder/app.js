...
MongoClient.connect(url,{ useNewUrlParser: true }, function(err, client) {
    ...
    app.post('/getData',(req,res,next) => {
        db.collection('data').find().limit(50).sort({_id: -1}).toArray(function (err, data) {
            res.send(data);
            res.end();
        })
    })
    ...
    app.get('/user',function (req, res) {
        ...
        if (!cookies.name) {
            ...
            db.collection("user").find(cookies).toArray(function(err, result) {
                console.log(result);
                if (result.length){
                    db.collection('data').find().limit(5).sort({_id : -1}).toArray(function (err, data) {
    ...
    app.get('/home', function (req, res) {
        ...
        // console.log(req.headers.cookie);
        // console.log(cookies.name);
        db.collection('data').find().limit(1).sort({_id : -1}).toArray(function (err, data) {
            ...
            if (!cookies.name) {
                ...
                db.collection("user").find(cookies).toArray( function (err, result) {
                    ...
                    else res.render('home',{data:data});
                })
            }
        })
    ...
    app.get('/getData/:humi/:temc/:temf', (req, res) => {
        var data= { humi: req.params.humi,
                    temc : req.params.temc,
                    temf : req.params.temf,
        }
